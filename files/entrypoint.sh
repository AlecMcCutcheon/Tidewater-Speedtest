...
if [ -f "$host_cert_path" ] && [ -f "$host_key_path" ]; then
    echo "Host-mounted certificates found. Using them directly."
    # Update nginx config to use host-mounted certificates
    sed -i "/\bssl_certificate\b/c\ssl_certificate \/host-certs\/live\/${DOMAIN_NAME}\/fullchain.pem;" "${CONFIG}"
    sed -i "/\bssl_certificate_key\b/c\ssl_certificate_key \/host-certs\/live\/${DOMAIN_NAME}\/privkey.pem;" "${CONFIG}"
    # Remove fallback certificate lines
    sed -i '/ssl_certificate \/etc\/ssl\/nginx.crt;/d' "${CONFIG}"
    sed -i '/ssl_certificate_key \/etc\/ssl\/nginx.key;/d' "${CONFIG}"
    nginx -s reload
    echo "Using host-mounted Let's Encrypt certificates."
else
    echo "Host-mounted certificates not found. Attempting to obtain new certificate..."
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -f "$fullchain_path" ]; then
            # Update nginx config to use container certificates
            ...
            sed -i '/ssl_certificate_key \/etc\/ssl\/nginx.key;/d' "${CONFIG}"
            nginx -s reload
            echo "Let's Encrypt certificate obtained successfully in container."
